...
The verified staging and package layout contains two native runtimes:
`runtimes/linux-x64/native/liblattice.so` and
`runtimes/win-x64/native/lattice.dll`. CI builds each from the exact source
recorded in `native/upstream.json`, verifies every managed import is exported,
runs the RID-specific ABI probe and native integration suite, and checks the
packaged bytes against the generated SHA-256 asset manifests.

The expanded suite passes all 22 tests on both Linux x64 and Windows x64,
including hard-kill recovery and subsequent writes. The assets are ready for a
